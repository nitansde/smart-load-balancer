...
- **Fill-first tie-breaking** — candidates tied on every ranking key keep natural ID order (`profile-2` before `profile-10`) and the head always wins, while sticky keeps each key on its profile while it stays healthy.
