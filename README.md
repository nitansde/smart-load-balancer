...
- **CPA-style rotation** — candidates tied on every ranking key rotate round-robin in natural ID order (CPA's default scheduler semantics: ID order, then round-robin within the tier), while sticky keeps each key on its profile while it stays healthy.
